...
Наша задача определить количество вариантов маршрутов, которыми лягушка может достичь риски под
номером N (натуральное число N вводится с клавиатуры).
...
"""
"""
при вводе 7 будет следующие итерации:
    get_path(6) + get_path(5) потом отталкиваемся  что num = 5
    get_path(4) + get_path(3) потом отталкиваемся  что num = 3
    get_path(2) + get_path(1) так как num равна единице останавливаем цикл
"""
...
def get_path(num):
    if num > 1:
        return get_path(num - 1) + get_path(num - 2) # В каждую следующую рекурсию мы заходим с num уменьшенным на два
    else:
        return 1
