...
Подвиг 5. (Для учебных целей). Вводится строка. Необходимо в ней заменить кириллические
символы на соответствующие латинские обозначения (без учета регистра букв),
...
def convert(str_in):
    """
    Функция которая преобразует русский символ в латинский
    :param str_in:
    :return:
    """
    ...
    str_out = ''
    for i in str_in.lower() :
        str_out += t.get(i) # t.get(i) возвращает значение по ключу

    return str_out


str_out = map(convert, str_in)

print(''.join(str_out))


# --------------------------- Еще одни варинат ---------------------------------------------
str_in_1 = input().lower()
t = {'ё': 'yo', 'а': 'a', 'б': 'b', 'в': 'v', 'г': 'g', 'д': 'd', 'е': 'e', 'ж': 'zh',
         'з': 'z', 'и': 'i', 'й': 'y', 'к': 'k', 'л': 'l', 'м': 'm', 'н': 'n', 'о': 'o', 'п': 'p',
         'р': 'r', 'с': 's', 'т': 't', 'у': 'u', 'ф': 'f', 'х': 'h', 'ц': 'c', 'ч': 'ch', 'ш': 'sh',
         'щ': 'shch', 'ъ': '', 'ы': 'y', 'ь': '', 'э': 'e', 'ю': 'yu', 'я': 'ya', ' ':'-'}
tp = list(map(lambda x: t[x] if x in t else '-', str_in_1))
print(''.join(tp))
