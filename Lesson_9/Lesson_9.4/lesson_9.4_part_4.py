...
money_S = set(map(int, input().split()))
money_G = set(map(int, input().split()))

different = money_S & money_G # Использовал пересечение множеств

result = sorted(filter(lambda x: x %2 == 0, different))
print(sorted(*result))
